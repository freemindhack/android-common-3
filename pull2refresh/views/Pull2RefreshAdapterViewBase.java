...
public abstract class Pull2RefreshAdapterViewBase <T extends AbsListView>
	...
	private static FrameLayout.LayoutParams
		...
		if (null != lp) {
			...
			if (lp instanceof LinearLayout.LayoutParams) {
				newLp.gravity =
					((LinearLayout.LayoutParams) lp).gravity;
			} else {
	 ...
	 * {@link Pull2RefreshBase#isPull2RefreshOverScrollEnabled()
	 * isPull2RefreshOverScrollEnabled()} returns false.
	 * 
	...
	public final void
		onScroll (final AbsListView view,
		...
		if (DEBUG) {
			Log.d(LOG_TAG, "First Visible: "
				+ firstVisibleItem + ". Visible Count: "
	...
	public final void onScrollStateChanged (
		final AbsListView view, final int state) {
		/**
	...
	public void setAdapter (ListAdapter adapter) {
		((AdapterView<ListAdapter>) mRefreshableView)
			.setAdapter(adapter);
	}
	...
	public final void setEmptyView (View newEmptyView) {
		...
		if (null != newEmptyView) {
			...
			ViewParent newEmptyViewParent =
				newEmptyView.getParent();
			if (null != newEmptyViewParent
				&& newEmptyViewParent instanceof ViewGroup) {
				((ViewGroup) newEmptyViewParent)
					.removeView(newEmptyView);
			}

			// We need to convert any LayoutParams so that it works in our
			// FrameLayout
			FrameLayout.LayoutParams lp =
				convertEmptyViewLayoutParams(newEmptyView
					.getLayoutParams());
			if (null != lp) {
				refreshableViewWrapper.addView(
					newEmptyView, lp);
			} else {
				refreshableViewWrapper
	...
	protected void handleStyledAttributes (TypedArray a) {
		...
		mShowIndicator =
			a.getBoolean(
				R.styleable.Pull2Refresh_ptrShowIndicator,
				!isPull2RefreshOverScrollEnabled());
	}
	...
	private void addIndicatorViews () {
		...
		if (mode.showHeaderLoadingLayout()
			&& null == mIndicatorIvTop) {
			// If the mode can pull down, and we don't have one set already
			mIndicatorIvTop =
				new IndicatorLayout(getContext(),
			...
			refreshableViewWrapper.addView(mIndicatorIvTop,
				params);
		...
		if (mode.showFooterLoadingLayout()
			...
			refreshableViewWrapper.addView(
				mIndicatorIvBottom, params);

		} else if (!mode.showFooterLoadingLayout()
			&& null != mIndicatorIvBottom) {
			// If we can't pull down, but have a View then remove it
			refreshableViewWrapper
				.removeView(mIndicatorIvBottom);
	...
	private boolean getShowIndicatorInternal () {
		return mShowIndicator && isPull2RefreshEnabled();
	}
	...
	private boolean isLastItemVisible () {
		...
		if (null == adapter || adapter.isEmpty()) {
			if (DEBUG) {
				Log.d(LOG_TAG,
					"isLastItemVisible. Empty View.");
			}
			return true;
		} else {
			final int lastItemPosition =
				mRefreshableView.getCount() - 1;
			final int lastVisiblePosition =
				mRefreshableView.getLastVisiblePosition();
	...
	private void removeIndicatorViews () {
		if (null != mIndicatorIvTop) {
			getRefreshableViewWrapper().removeView(
				mIndicatorIvTop);
			mIndicatorIvTop = null;
		}
