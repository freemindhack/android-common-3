...
import nocom.pull2refresh.interfaces.ILoadingLayout;
import nocom.pull2refresh.interfaces.IPull2Refresh;
...
public abstract class Pull2RefreshBase <T extends View>
	extends LinearLayout implements IPull2Refresh<T> {
	...
	static final String STATE_CURRENT_MODE =
		"ptr_current_mode";
	static final String STATE_SCROLLING_REFRESHING_ENABLED =
		"ptr_disable_scrolling";
	static final String STATE_SHOW_REFRESHING_VIEW =
		"ptr_show_refreshing_view";
	...
	public void addView (View child, int index,
		...
		if (refreshableView instanceof ViewGroup) {
			((ViewGroup) refreshableView).addView(child,
				index, params);
		} else {
	...
	public final ILoadingLayout getLoadingLayoutProxy (
		...
		return createLoadingLayoutProxy(includeStart,
			includeEnd);
	}
	...
	@Override
	public final boolean isPull2RefreshEnabled () {
		return mMode.permitsPull2Refresh();
	}


	@Override
	public final boolean isPull2RefreshOverScrollEnabled () {
	...
	public final boolean onInterceptTouchEvent (
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (isReadyForPull()) {
				...
				// direction
				switch (getPull2RefreshScrollDirection()) {
				case HORIZONTAL:
					diff = x - mLastMotionX;
	...
	@Override
	public final void setFilterTouchEvents (
	 ...
	 */
	public final void
		setPull2RefreshEnabled (boolean enable) {
		setMode(enable ? Mode.getDefault() : Mode.DISABLED);
	}


	@Override
	public final void setPull2RefreshOverScrollEnabled (
		boolean enabled) {
	 ...
	 */
	public void setRefreshingLabel (
		CharSequence refreshingLabel, Mode mode) {
		getLoadingLayoutProxy(
			mode.showHeaderLoadingLayout(),
			mode.showFooterLoadingLayout())
	...
	@Override
	public final void setShowViewWhileRefreshing (
		boolean showView) {
	...
	protected LoadingLayout createLoadingLayout (
		...
		LoadingLayout layout =
			mLoadingAnimationStyle.createLoadingLayout(
				context, mode,
				getPull2RefreshScrollDirection(), attrs);
	...
	protected int getPull2RefreshScrollDuration () {
		return SMOOTH_SCROLL_DURATION_MS;
	}


	protected int getPull2RefreshScrollDurationLonger () {
	 ...
	 * 
	 * @param a - TypedArray of Pull2Refresh Attributes
	 */
	 ...
	 */
	protected void onPtrRestoreInstanceState (
	 ...
	 */
	protected void
		onPtrSaveInstanceState (Bundle saveState) {
	}
	...
	protected final void onRestoreInstanceState (
		...
		if (state instanceof Bundle) {
			...
			setMode(Mode.mapIntToValue(bundle.getInt(
				STATE_MODE, 0)));
			mCurrentMode =
				Mode.mapIntToValue(bundle.getInt(
					STATE_CURRENT_MODE, 0));
			...
			super.onRestoreInstanceState(bundle
				.getParcelable(STATE_SUPER));

			State viewState =
				State.mapIntToValue(bundle.getInt(
					STATE_STATE, 0));
	...
	protected final Parcelable onSaveInstanceState () {
		...
		bundle.putInt(STATE_CURRENT_MODE,
			mCurrentMode.getIntValue());
		bundle.putBoolean(
			STATE_SCROLLING_REFRESHING_ENABLED,
			mScrollingWhileRefreshingEnabled);
		bundle.putBoolean(STATE_SHOW_REFRESHING_VIEW,
			mShowViewWhileRefreshing);
		bundle.putParcelable(STATE_SUPER,
	...
	@Override
	protected final void onSizeChanged (int w, int h,
		int oldw, int oldh) {
		if (DEBUG) {
			Log.d(LOG_TAG, String.format(
				"onSizeChanged. W: %d, H: %d", w, h));
		}
	...
	protected final void refreshLoadingViewsSize () {
		final int maximumPullScroll =
			(int) (getMaximumPullScroll() * 1.2f);
	...
	protected final void refreshRefreshableViewSize (
		int width, int height) {
		// We need to set the Height of the Refreshable View to the same as
		// this layout
		LinearLayout.LayoutParams lp =
			(LinearLayout.LayoutParams) mRefreshableViewWrapper
				.getLayoutParams();
	...
	public final void setHeaderScroll (int value) {
		...
		value =
			Math.min(maximumPullScroll,
				Math.max(-maximumPullScroll, value));
		...
		if (USE_HW_LAYERS) {
			...
			ViewCompat.setLayerType(
				mRefreshableViewWrapper, value != 0
	...
	protected final void smoothScrollTo (int scrollValue,
		OnSmoothScrollFinishedListener listener) {
		smoothScrollTo(scrollValue,
			getPull2RefreshScrollDuration(), 0, listener);
	}
	 ...
	 * @param scrollValue - Position to scroll to
	 */
	protected final void smoothScrollToLonger (
		int scrollValue) {
		smoothScrollTo(scrollValue,
			getPull2RefreshScrollDurationLonger());
	}
	...
	protected void updateUIForMode () {
		...
		// set it to pull down
		mCurrentMode =
			(mMode != Mode.BOTH) ? mMode
				: Mode.PULL_FROM_START;
	}


	private void addRefreshableView (Context context,
		T refreshableView) {
	...
	private void init (Context context, AttributeSet attrs) {
		...
		TypedArray a =
			context.obtainStyledAttributes(attrs,
				R.styleable.Pull2Refresh);

		if (a.hasValue(R.styleable.Pull2Refresh_ptrMode)) {
			mMode =
				Mode.mapIntToValue(a.getInteger(
					R.styleable.Pull2Refresh_ptrMode, 0));
		}

		if (a
			.hasValue(R.styleable.Pull2Refresh_ptrAnimationStyle)) {
			mLoadingAnimationStyle =
				AnimationStyle
					.mapIntToValue(a
						.getInteger(
							R.styleable.Pull2Refresh_ptrAnimationStyle,
							0));
		}
		 ...
		 */
		if (a
			.hasValue(R.styleable.Pull2Refresh_ptrRefreshableViewBackground)) {
			Drawable background =
				a.getDrawable(R.styleable.Pull2Refresh_ptrRefreshableViewBackground);
			if (null != background) {
				mRefreshableView
					.setBackgroundDrawable(background);
			}
		} else if (a
			.hasValue(R.styleable.Pull2Refresh_ptrAdapterViewBackground)) {
			Utils.warnDeprecation(
				"ptrAdapterViewBackground",
				"ptrRefreshableViewBackground");
			Drawable background =
				a.getDrawable(R.styleable.Pull2Refresh_ptrAdapterViewBackground);
			if (null != background) {
				mRefreshableView
					.setBackgroundDrawable(background);
			}
		...
		if (a
			.hasValue(R.styleable.Pull2Refresh_ptrOverScroll)) {
			mOverScrollEnabled =
				a.getBoolean(
					R.styleable.Pull2Refresh_ptrOverScroll,
					true);
		}

		if (a
			.hasValue(R.styleable.Pull2Refresh_ptrScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled =
				a.getBoolean(
					R.styleable.Pull2Refresh_ptrScrollingWhileRefreshingEnabled,
					false);
		}
	...
	private void pullEvent () {
		...
		case PULL_FROM_END:
			newScrollValue =
				Math.round(Math.max(initialMotionValue
					- lastMotionValue, 0)
					/ FRICTION);
		...
		default:
			newScrollValue =
				Math.round(Math.min(initialMotionValue
					- lastMotionValue, 0)
					/ FRICTION);
	...
	private final void smoothScrollTo (int newScrollValue,
		...
		if (oldScrollValue != newScrollValue) {
			...
			if (delayMillis > 0) {
				postDelayed(mCurrentSmoothScrollRunnable,
					delayMillis);
			} else {
	...
	public static enum AnimationStyle {
		/**
		 * This is the default for Android-Pull2Refresh. Allows you to use any
		...
		LoadingLayout createLoadingLayout (Context context,
			Mode mode, Orientation scrollDirection,
			TypedArray attrs) {
			switch (this) {
			case ROTATE:
			default:
	...
	public static enum Mode {
		 ...
		 */
		public boolean permitsPull2Refresh () {
	...
	public static interface OnPullEventListener <V extends View> {
		...
		public void onPullEvent (
			final Pull2RefreshBase<V> refreshView,
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run () {
			...
			if (mStartTime == -1) {
				...
				normalizedTime =
					Math.max(
						Math.min(normalizedTime, 1000), 0);
