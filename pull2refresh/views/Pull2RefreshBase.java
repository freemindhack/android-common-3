...
public abstract class Pull2RefreshBase <T extends View> extends LinearLayout
	...
	@Override
	public final LoadingLayoutInterface getLoadingLayoutProxy () {
	 ...
	 *             the passed value.
	 */
	public final void setPull2RefreshEnabled (boolean enable) {
		setMode(enable ? Mode.getDefault() : Mode.DISABLED);
	}


	@Override
	public final void setPull2RefreshOverScrollEnabled (boolean enabled) {
	 ...
	 *         direction.
	 */
	public abstract Orientation getPull2RefreshScrollDirection ();
	...
	protected LoadingLayout createLoadingLayout (Context context, Mode mode,
		...
		LoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(
			context, mode, getPull2RefreshScrollDirection(), attrs);
	 ...
	 * 
	 * @param a
	 *            - TypedArray of Pull2Refresh Attributes
	 */
	...
	protected void onPull2Refresh () {
		switch (mCurrentMode) {
		case PULL_FROM_END:
			mFooterLayout.Pull2Refresh();
			break;
		case PULL_FROM_START:
			mHeaderLayout.Pull2Refresh();
			break;
		default:
			// NO-OP
			break;
	...
	protected final void onRestoreInstanceState (Parcelable state) {
		if (state instanceof Bundle) {
			...
			mCurrentMode = Mode.mapIntToValue(bundle.getInt(
			...
			State viewState = State.mapIntToValue(bundle.getInt(STATE_STATE,
				0));
	...
	public final void setHeaderScroll (int value) {
		...
		value = Math.min(maximumPullScroll,
	...
	protected final void smoothScrollTo (int scrollValue) {
		smoothScrollTo(scrollValue, getPull2RefreshScrollDuration());
	}
	...
	protected final void smoothScrollTo (int scrollValue,
		...
		smoothScrollTo(scrollValue, getPull2RefreshScrollDuration(), 0,
			listener);
	}
	...
	protected final void smoothScrollToLonger (int scrollValue) {
		smoothScrollTo(scrollValue, getPull2RefreshScrollDurationLonger());
	}
	...
	private void init (Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.Pull2Refresh_mode)) {
			mMode = Mode.mapIntToValue(a.getInteger(
				R.styleable.Pull2Refresh_mode, 0));
		}
		if (a.hasValue(R.styleable.Pull2Refresh_animationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a
				.getInteger(R.styleable.Pull2Refresh_animationStyle, 0));
		}
		// Refreshable View
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.Pull2Refresh_refreshableViewBackground)) {
			Drawable background = a
				.getDrawable(R.styleable.Pull2Refresh_refreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(R.styleable.Pull2Refresh_adapterViewBackground)) {
			Utils.warnDeprecation("adapterViewBackground",
				"refreshableViewBackground");
			Drawable background = a
				.getDrawable(R.styleable.Pull2Refresh_adapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}
		if (a.hasValue(R.styleable.Pull2Refresh_overScroll)) {
			mOverScrollEnabled = a.getBoolean(
				R.styleable.Pull2Refresh_overScroll, true);
		}
		if (a
			.hasValue(R.styleable.Pull2Refresh_scrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(
				R.styleable.Pull2Refresh_scrollingWhileRefreshingEnabled,
				false);
		}
	...
	public static enum AnimationStyle {
		 ...
		 * drawable, which is automatically rotated and used as a Progress
		 * Bar.
		 */
		ROTATE,
		/**
	...
	public static interface OnPullEventListener <V extends View> {
		 ...
		 */
		public void onPullEvent (final Pull2RefreshBase <V> refreshView,
