...
public abstract class SocketClient implements
	...
	private int socketClientConnectState =
		SocketClient.SOCKET_STATE_IDLE;
	private ConnectArguments connectArguments =
		new ConnectArguments("");
	private MyQueue<SendRecvData> newSendBuffers =
		new MyQueue<SendRecvData>();
	private MyQueue<SendRecvData> waitAckBuffers =
		new MyQueue<SendRecvData>();
	...
	private OnSocketStateChanged onSocketStateChanged =
		null;
	...
	public boolean sendData (SendRecvData data,
		...
		try {
			...
			if (sendNow) {
				...
				try {
					thisOutputStream.write(data.data, 0,
						data.data.length);
			...
			try {
				synchronized (this) {
					...
					while (true) {
						...
						if (ret) {
							...
							Log.println(
								...
								"Connected");
							socketClientConnectState =
								SocketClient.SOCKET_STATE_CONNECT_ONLINE;
							onReceiveThread =
								new Thread(receiveRuntime);
							onReceiveThread.start();
							onSendThread =
								new Thread(sendRuntime);
							onSendThread.start();
						} else {
							socketClientConnectState =
								SocketClient.SOCKET_STATE_CONNECT_TIMEOUT;
					...
					if (null != stateChangedHandler) {
						...
						msg.what =
							SocketClient.connectStateKey;
						bundle.clear();
						bundle
							.putInt(
								SocketClient.connectStateKeyStr,
								socketClientConnectState);
						msg.setData(bundle);
						try {
							stateChangedHandler
				...
				UNLOCK();
				Log.println(Log.VERBOSE,
					"SocketClient/socketRuntime", "Exited");
			} catch (Exception e) {
				try {
					UNLOCK();
	...
	Runnable receiveRuntime = new Runnable() {
		public void run () {
			try { /* all of this func */
				...
				synchronized (this) {
					while (true) {
						...
						if ((null == socketClient)
							|| (!socketClient.isConnected())) {
							socketClientConnectState =
								SocketClient.SOCKET_STATE_RECV_BROKEN;
							break;
						}
						...
						if (dataReady > 0) {
							boolean onlyWhenNoAvailable =
								isUseBEOnlyNoAvailable();
							...
							try {
								count =
									thisInputStream.read(
										buffer, 0, 1024);
								// count = inputStream.read(buffer);
							} catch (Exception e) {
								Log.e(
									"SockectClient/receiveRuntime",
									e.getMessage());
								socketClientConnectState =
							...
							if (count > 0) {
								...
								SendRecvData sendRecvData =
									getSendRecvDataRM(recvedDataId);
				...
				Disconnect();
				Log.println(Log.VERBOSE,
					"SocketClient/receiveRuntime", "Exited");
			} catch (Exception e) {
	...
	Runnable sendRuntime = new Runnable() {
		public void run () {
			try { /* all of this func */
				int prompt = 0;
				boolean unlock;
				MyQueue<SendRecvData> dump2Send =
					new MyQueue<SendRecvData>();
				...
				synchronized (this) {
					while (true) {
						...
						if ((null == socketClient)
							|| (!socketClient.isConnected())) {
							socketClientConnectState =
								SocketClient.SOCKET_STATE_SEND_BROKEN;
							break;
						}

						if (newSendBuffers.count() > 0) {
							dump2Send
								.appendEmpty(newSendBuffers);
						}
				...
				Disconnect();
				Log.println(Log.VERBOSE,
					"SocketClient/sendRuntime", "Exited");
			} catch (Exception e) {
				/* end of all of this func */
				try {
					UNLOCK();
	...
	protected SendRecvData getSendRecvDataRM (
		byte[] recvedDataId) {
		Log.println(Log.VERBOSE, "getSendRecvDataRM",
			"getSendRecvDataRM");
		...
		if (sz <= 0) {
			...
			if (null == srd) {
				Log.println(Log.INFO,
					"getSendRecvDataRM/find", "NOT FOUND");
			} else {
				Log.println(Log.INFO,
					"getSendRecvDataRM/find", "FOUND");
			}
			return srd;
		}
	}
		...
		try {
			...
			if (null != socketClient) {
				...
				try {
					...
					Log.println(
						Log.ERROR,
						"DisconnectFromServer/shutdownInput",
						e.getMessage());
				}
			...
			this.socketClientConnectState =
				SocketClient.SOCKET_STATE_FI_DESTROYED;
