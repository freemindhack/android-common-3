...
public class MyWifiManager implements
	MyWifiManagerInterface {
	...
	public boolean isWifiConnected () {
		try {
			ConnectivityManager manager =
				(ConnectivityManager) this.savedContext
	...
	public boolean openWifi () {
		try { /* for all of this func */
			...
			Log.println(Log.ERROR,
				"MyWifiManager/openWifi", e.getMessage());
			// this.closeWifi();
			return false;
	...
	public void closeWifi () {
		try { /* for all of this func */
			...
			Log.println(Log.ERROR,
				"MyWifiManager/closeWifi", e.getMessage());
		} /* end of all of this func */
	} /* end of closeWifi */
	...
	public boolean startScan (int retry) {
		try { /* for all of this func */
			...
			this.reachableWifisList =
				this.wifiManager.getScanResults();

			return true;
		} catch (Exception e) {
			Log.println(Log.ERROR,
				"MyWifiManager/startScan", e.getMessage());
			return false;/* error */
		} /* end of all of this func */
	} /* end of startScan */
	...
	public StringBuilder lookupScan () {
		try { /* for all of this func */
			...
			for (int i = 0; i < this.reachableWifisList
				.size(); i++) {
				stringBuilder.append("Index_"
					+ new Integer(i + 1).toString() + ":");

				stringBuilder
					.append((this.reachableWifisList.get(i))
						.toString());
			...
			Log.println(Log.ERROR,
				"MyWifiManager/lookupScan", e.getMessage());
			return null;/* error */
		} /* end of all of this func */
	} /* end of lookupScan */
	...
	public String getMacAddress () {
		try { /* for all of this func */
			...
			return (wifiInfo == null) ? "0:0:0:0:0:0"
				: wifiInfo.getMacAddress();
		} catch (Exception e) {
			Log.println(Log.ERROR,
				"MyWifiManager/getMacAddress",
				e.getMessage());
	...
	public String getBSSID () {
		try { /* for all of this func */
			WifiInfo wifiInfo =
				this.wifiManager.getConnectionInfo();
			return (wifiInfo == null) ? "" : wifiInfo
				.getBSSID();
		} catch (Exception e) {
			Log.println(Log.ERROR,
				"MyWifiManager/getBSSID", e.getMessage());
	...
	public int getIPAddress () {
		try { /* for all of this func */
			WifiInfo wifiInfo =
				this.wifiManager.getConnectionInfo();
			return (wifiInfo == null) ? 0 : wifiInfo
				.getIpAddress();
		} catch (Exception e) {
			Log.println(Log.ERROR,
				"MyWifiManager/getIPAddress",
				e.getMessage());
	...
	public String getWifiInfo () {
		try { /* for all of this func */
			...
			Log.println(Log.ERROR,
				"MyWifiManager/getWifiInfo", e.getMessage());
			return "";/* error not got */
		} /* end of all of this func */
	} /* getWifiInfo */
	...
	/* @param: ssid: no '\"' and '\"'. */
	@Override
	public boolean isWifiExsitsInSavedList (String ssid,
		...
		try { /* for all of this func */
			...
			Log.println(Log.ERROR,
				"MyWifiManager/isWifiExsitsInSavedList",
				e.getMessage());
			return false;/* error */
		} /* end of all of this func */
	...
	/* @param: ssid: no '\"' and '\"'. */
	@Override
	public boolean isWifiExsitsInAll (String ssid,
		boolean withQuotation) {
		try {
			boolean ret =
				this.isWifiExsitsInSavedList(ssid,
					withQuotation);
			...
			return this.isWifiExsitsInReachableList(ssid,
				withQuotation);
		} catch (Exception e) {
			return false;
		}
	...
	/* @param: ssid: no '\"' and '\"'. */
	@Override
	public boolean isWifiExsitsInReachableList (
		String ssid, boolean withoutQuotation) {
		try {
			boolean ret = this.startScan(64);
			if (!ret) {
				return ret;
			...
			for (ScanResult reachableConfig : this.reachableWifisList) {
				Log.println(Log.VERBOSE,
					"isWifiExsitsInReachableList", "ssid: "
						+ reachableConfig.SSID);
	...
	public boolean connect2Wifi (
		...
		try { /* for all of this func */
			...
			ret =
				this.wifiManager.enableNetwork(netId,
					disableOthers);
			Log.println(Log.VERBOSE,
				"MyWifiManager/connect2Wifi", "netId: "
					+ netId + "; " + "enabled: " + ret);
			return ret;
		} catch (Exception e) {
			Log.println(Log.ERROR,
				"MyWifiManager/connect2Wifi",
				e.getMessage());
			return false;/* error */
		} /* end of all of this func */
	} /* connect2Wifi */
	...
	@Override
	public boolean connect2Wifi (boolean oldEnabled,
		String oldssid, String newssid,
		boolean disableOthers, boolean removeWhenExist) {
		try { /* for all of this func */
			...
			if ((null != oldssid) && (oldssid.length() > 0)) {
				cfg =
					this.getWifiConfiguration(oldssid,
						false);
				// oldid = this.wifiManager.addNetwork(cfg);
				oldid = cfg.networkId;
				ret =
					this.wifiManager.disableNetwork(oldid);
			...
			ret =
				this.isWifiExsitsInSavedList(cfg.SSID, true);

			if (ret && removeWhenExist) {
				this.wifiManager
					.disableNetwork(cfg.networkId);
			...
			ret =
				this.wifiManager.enableNetwork(newid,
					disableOthers);

			if (!ret) {
				if (oldEnabled && (null != oldssid)
					&& (oldssid.length() > 0)) {
					this.wifiManager.enableNetwork(oldid,
						disableOthers);
				}
				return false;
			}
	...
	@Override
	public boolean connect2Wifi (
		int whichwifiConfiguration, boolean disableOthers,
		boolean removeWhenExist) {
		try { /* for all of this func */
			List<WifiConfiguration> savedConfigs =
				this.wifiManager.getConfiguredNetworks();
			if ((whichwifiConfiguration < 0)
				|| (null == savedConfigs)
				|| (whichwifiConfiguration > savedConfigs
					.size())) {
				return false;
			}
			return this.wifiManager
				.enableNetwork(savedConfigs
					.get(whichwifiConfiguration).networkId,
					disableOthers);
		} catch (Exception e) {
			Log.println(Log.ERROR,
				"MyWifiManager/connect2Wifi",
	...
	public boolean removeNetwork (String ssid,
		boolean restoreSaved, boolean disableOthers) {
		try {
			try {
				WifiConfiguration cfg = null;

				cfg =
					this.getWifiConfiguration(ssid, false);
				if (null != cfg) {
					this.removeNetwork(cfg);
				}
			} catch (Exception e) {
	...
	public WifiConfiguration createWifiConfiguration (
		...
		try { /* for all of this func */
			...
			Log.println(Log.VERBOSE, "createWifiInfo",
				"ssid: " + config.SSID);
			...
			Log.println(Log.ERROR,
				"MyWifiManager/WifiConfiguration",
				e.getMessage());
			return null;/* error */
		} /* end of all of this func */
	...
	public WifiConfiguration getWifiConfiguration (
		String ssid, boolean withQuotation) {
		try { /* for all of this func */
			List<WifiConfiguration> savedConfigs =
				this.wifiManager.getConfiguredNetworks();
			...
			Log.println(Log.ERROR,
				"MyWifiManager/getWifiConfiguration",
				"ABORT");
			return null;/* error */
		} /* end of all of this func */
	...
	public String getWifiSsid () {
		try {
			...
			Log.println(Log.ERROR,
				"NetworkUtils/getWifiSSID", e.getMessage());
			return "";
		}
	}
	...
	public void saveWifiState () {
		try {
			...
			this.savedWifiConfiguration =
				this.getWifiConfiguration(
					this.getWifiSsid(), true);
			Log.println(Log.VERBOSE, "saveWifiState",
				"WIFI_USE_STATIC_IP: "
					+ this.savedStaticNetowrk);
		} catch (Exception e) {
			;
	...
	public void restoreWifiState (boolean disableOthers) {
		try {
			Log.println(Log.INFO, "restoreWifiState",
				"restoreWifiState");
			...
			if (null != savedStaticNetowrk
				&& savedStaticNetowrk.length() > 0) {
				android.provider.Settings.System
					.putString(
						this.savedContext
							.getContentResolver(),
			...
			Log.println(Log.WARN, "restoreWifiState",
				e.getMessage());
		}
	}
	...
	@SuppressWarnings ("deprecation")
	@Override
	public void setStaticNetwork (String ip,
		String gateway, String netmask, List<String> dns) {
		try {
			android.provider.Settings.System
				.putString(
